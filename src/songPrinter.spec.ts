...
import {
  ...
  SONG_WITH_SUB_SECTIONS_THAT_REQUIRE_EXTRA_SPLIT_MOCK_FILE_CONTENT,
...
`);
  ...
  it('should not adjust an already well structured song (w/ subsections)', () => {
    expect(print(parse(SONG_WITH_SUBSECTIONS_MOCK_FILE_CONTENT))).
...
`);
  ...
  it('should correctly re-split a song split w/ further required modifications', () => {
    expect(
print(
parse(
SONG_WITH_SUB_SECTIONS_THAT_REQUIRE_EXTRA_SPLIT_MOCK_FILE_CONTENT))).
...
toMatchInlineSnapshot(`
"[title]
My custom title {author: {Betania Dublin}, alternative: {Când eram fără speranță}, version: {ii}, contentHash: {95a5cd}, id: {7RURbpko41pWYEgVkHD4Pq}}

[sequence]
c,v1.1,v1.2,c,v2,c,v3,c
...
`);
  ...
  it('should correctly add only the unique sections of a song', () => {
    expect(() =>
      print(parse(createSongMock(['v1', 'c', 'v2', 'c']))),
