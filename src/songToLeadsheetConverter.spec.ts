...
Cântă, suflet al meu, pentru Dumnezeu {alternative: {*}, composer: {Matt Redman}, writer: {*}, arranger: {*}, interpreter: {Alin Timofte}, band: {*}, key: {*}, tempo: {*}, tags: {*}, version: {*}, genre: {*}, rcId: {164159}, id: {abCwAsZpAWGVzGNBWEfyhB}, contentHash: {1befcd}}
...
Cum ^{G}nu ai ^{A}mai cân^{Dsus-D}tat.",
    ...
    "[title]": {
      "content": "Cântă, suflet al meu, pentru Dumnezeu {alternative: {*}, composer: {Matt Redman}, writer: {*}, arranger: {*}, interpreter: {Alin Timofte}, band: {*}, key: {*}, tempo: {*}, tags: {*}, version: {*}, genre: {*}, rcId: {164159}, id: {abCwAsZpAWGVzGNBWEfyhB}, contentHash: {1befcd}}",
...
`);
  ...
  describe('getNormalizedContent', () => {
    ...
    it('should normalise word with more than three chord notations correctly', () => {
      expect(getNormalizedContent('^{G}Te-o^{D}no^{A}rez')).toEqual('^*{G}Te-o ^*{D}no ^{A}rez');
    });
